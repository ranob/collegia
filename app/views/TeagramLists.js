// Let's create an instance of the overlay which is used to show the full size image when a user
// taps on one of the list items.
// Note:  we are not keeping it inside the same namespace as TeagramInnerList just to show that it is still "just JavaScript".
var overlay = new Ext.Panel({
    // We'll set the image src attribute's value programmatically.
    tpl: "<tpl for='.'><div class='gram-container'><img id='current-gram' /><p>{id}</p><p>{name}</p><p>{surname}</p></div></tpl>",
    floating: true,
    modal: true,
    centered: true,
    width: 658,
...
teagrams.views.TeagramInnerList = Ext.extend(Ext.List, {
    ...
    itemTpl: teagrams.views.innerListItemTpl(),
                   ...
                   store: new Ext.data.Store({
                       ...
                       proxy: {
                          ...
                          // If you're offline, uncomment the two lines below and comment the line above and the line with the Instagram URL.
                           type: 'ajax',
                           url: 'app/models/person.json',
                          // url: 'http://localhost:7070/controller/services/people/getPupilsByPartialName?query=alumno',
                           reader: {
                               type: 'json',
